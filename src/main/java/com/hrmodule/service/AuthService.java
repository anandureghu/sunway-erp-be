package com.hrmodule.service;

import com.hrmodule.domain.Role;
import com.hrmodule.domain.User;
import com.hrmodule.dto.auth.LoginRequest;
import com.hrmodule.dto.auth.RegisterRequest;
import com.hrmodule.repo.UserRepository;
import com.hrmodule.security.JwtService;
import org.springframework.security.crypto.password.PasswordEncoder;
import org.springframework.stereotype.Service;
import java.util.Map;

@Service
public class AuthService {
    private final UserRepository userRepository;
    private final PasswordEncoder encoder;
    private final JwtService jwt;

    public AuthService(UserRepository userRepository, PasswordEncoder encoder, JwtService jwt) {
        this.userRepository = userRepository; this.encoder = encoder; this.jwt = jwt;
    }

    public User register(RegisterRequest req) {
        if (userRepository.existsByEmail(req.getEmail())) throw new IllegalArgumentException("Email already in use");
        if (userRepository.existsByUsername(req.getUsername())) throw new IllegalArgumentException("Username already in use");
        User u = new User();
        u.setFullName(req.getFullName());
        u.setEmail(req.getEmail());
        u.setUsername(req.getUsername());
        u.setPassword(encoder.encode(req.getPassword()));
        u.setRole(req.getRole() == null ? Role.USER : req.getRole());
        return userRepository.save(u);
    }

    public Map<String, String> login(LoginRequest req) {
        User u = userRepository.findByEmail(req.getLoginId())
                .or(() -> userRepository.findByUsername(req.getLoginId()))
                .orElseThrow(() -> new IllegalArgumentException("Invalid credentials"));
        if (!encoder.matches(req.getPassword(), u.getPassword())) throw new IllegalArgumentException("Invalid credentials");
        String access = jwt.generateAccessToken(u.getUsername(), Map.of("username", u.getUsername(), "role", u.getRole().name(), "uid", u.getId()));
        String refresh = jwt.generateRefreshToken(u.getUsername());
        return Map.of("accessToken", access, "refreshToken", refresh);
    }

    public Map<String, String> refresh(String refreshToken) {
        var claims = jwt.parse(refreshToken).getBody();
        String username = claims.getSubject();
        User u = userRepository.findByUsername(username).orElseThrow();
        String access = jwt.generateAccessToken(u.getUsername(), Map.of("username", u.getUsername(), "role", u.getRole().name(), "uid", u.getId()));
        String refresh = jwt.generateRefreshToken(u.getUsername());
        return Map.of("accessToken", access, "refreshToken", refresh);
    }
}
